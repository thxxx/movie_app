...

{
    ...
    */
    ...
    //makeTwoProcess(arguments, maxlen, number);
    makeTwoProcess(arguments, maxlen, number);
...
int fm2(char **args, int maxlen, int number)
{
    ...
    if (maxlen <= nowChilderen)
    {
        ...
        { //child process
            ...
            char *argv[] = {"program1", *args, (char *)0}; // merge sort로 넘겨줄 인자들. program1의 redirection입력으로 안넘어간다.
    ...
    else // 아직 생성된 프로세스 수가 maxlen보다 작다면
    {
        makeTwoProcess(args, maxlen, number);
    }
